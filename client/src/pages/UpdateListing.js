...
const handleSubmit=async(e)=>{
    ...
    try {
        ...
        if(formdata.address === ''){
            return toast.error('Please enter address location ');
        }
  ...
  return (
    ...
    <div className='flex flex-col gap-5 mb-5'>
            <input type="text" placeholder='Title' id='name' minLength="8" maxLength="50" className='border rounded-lg p-3 w-80  focus:outline-none sm:w-96' onChange={handleOnChange} value={formdata.name}/>
        ...
        <div className='flex gap-2'>
            <input type="number" id='regularprice' min={50}  required className='p-3 border border-gray-300 rounded-lg' onChange={handleOnChange} value={formdata.regularprice}/>
    ...
    <div className='flex flex-col  flex-1 gap-4  p-3 w-80  focus:outline-none sm:w-96'>
        ...
        <div className='flex gap-4'>
            ...
            <button type='button' disabled={upload} onClick={handleImageSubmit} className='uppercase text-green-600 border  border-green-600 p-1 rounded font-semibold'>{upload ? 'uploading' : 'upload'}</button>
        </div>
    ...
    <button disabled={loading} className='p-3 uppercase rounded-lg bg-blue-950 text-white hover: opacity-95  disabled:opacity-80'>{loading ? 'Loading' : 'Update Listing'}</button>
