...
export default function Header() {
  ...
  {currentUser ?( <Link to="/profile"> <img src={currentUser.avatar} width={40} height={40} className='rounded-full' alt="profile" /></Link>)
    ...
    <button data-collapse-toggle="navbar-default" type="button" id='toggle-btn' class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-700 rounded-lg sm:hidden  focus:outline-none focus:ring-2 focus:ring-gray-200" aria-controls="navbar-default" aria-expanded="false" onClick={()=>{toggleMenu()}}>
