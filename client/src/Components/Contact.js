...
export default function Contact({listing}) { 
  ...
  const [formdata, setFormdata] = useState({username: currentUser.username, email:currentUser.email, email2:owner.email, phone: '', message: ''});
...
const sendContactDetails = async()=>{
  ...
  return (
    ...
    <div className='hidden' id="contact2">
      <span  className='uppercase bg-green-700 rounded text-lg text-white text-center w-96 p-2 hover:opacity-90'>Send Message</button> 
        </div> 
